...
class AlgorithmConfig:
    ...
    num_ally = 'auto load, do not change'
    num_opp = 'auto load, do not change'
...
class ReinforceAlgorithmFoundation(RLAlgorithmBase):
    def __init__(self, n_agent, n_thread, space, mcv=None, team=None):
        ...
        # heterogeneous agent types & type_mask 因为整个训练中type_mask不变，于是放到此处
        agent_type_list = [a['type'] for a in GlobalConfig.ScenarioConfig.SubTaskConfig.agent_list]
        AI_agent_type_list = [a['type'] for a in GlobalConfig.ScenarioConfig.SubTaskConfig.agent_list if a['team']==0]
        self.HeteAgentType = str_array_to_num(agent_type_list)
        self.AIHeteAgentType = str_array_to_num(AI_agent_type_list)
        AlgorithmConfig.HeteAgentType = self.HeteAgentType
        AlgorithmConfig.num_type = len(set(self.HeteAgentType))
        ...
        # initialize policy
        AlgorithmConfig.n_action = n_actions
    ...
    def action_making(self, StateRecall, test_mode):
        ...
        if self.CODE_ACTION is None:
            shape = [obs.shape[0], obs.shape[1], AlgorithmConfig.n_action]
            self.CODE_ACTION = np.zeros(shape)
        if self.GRU_CELL is None:
            shape = [obs.shape[0], obs.shape[1], AlgorithmConfig.net_hdim]
            # self.GRU_CELL = np.random.rand(*shape)
            self.GRU_CELL = np.zeros(shape)

        # 从上一步存储中提取当前激活线程中的信息
        previous2now_code_action = np.zeros([obs.shape[0], obs.shape[1], AlgorithmConfig.n_action])
        previous2now_gru_cell_memory = np.zeros([obs.shape[0], obs.shape[1], AlgorithmConfig.net_hdim])
        j = 0
        for i, is_active in enumerate(threads_active_flag):
            if is_active:
                previous2now_code_action[j] = self.CODE_ACTION[i]
                previous2now_gru_cell_memory[j] = self.GRU_CELL[i]
                j += 1

        # make decision
        with torch.no_grad():
            action, value, action_log_prob, gru_cell_output = self.policy.act(obs=obs,
                                                             action_code=previous2now_code_action,
                                                             gru_cell_memory=previous2now_gru_cell_memory,
                                                             test_mode=test_mode,
                                                             avail_act=avail_act,
                                                             eprsn=eprsn,
                                                             )

        action_one_hot = np.eye(AlgorithmConfig.n_action)[action.flatten()].reshape(obs.shape[0], obs.shape[1], AlgorithmConfig.n_action)
        ...
        for i, is_active in enumerate(threads_active_flag):
            if is_active:
                ...
                self.GRU_CELL[i] = gru_cell_output[j]
                j += 1
        
        # 制造GRL预测模块使用的标签
        if self.type_label is None:
            temp_arr = np.array(self.AIHeteAgentType)
            temp_arr = np.expand_dims(temp_arr, axis=0)  
            self.type_label = np.tile(temp_arr, (obs.shape[0], 1))  
    ...
    def load_model(self, AlgorithmConfig):
        ...
        if AlgorithmConfig.load_checkpoint:
            ...
            assert manual_dir != '', "This type of load pt should have a specific checkpoint!"
            ...
            # https://*******.com/pytorch/pytorch/issues/3852
