...
class PPO():
    def __init__(self, policy_and_critic, ppo_config, mcv=None):
        ...
        self.predict_coef = ppo_config.predict_coef
    ...
    def establish_pytorch_graph(self, flag, sample, n):
        ...
        gru_cell_memory = _2tensor(sample['gru_cell_memory'])
        ...
        type_predict = type_predict.view(-1,type_predict.shape[-1])
        type_label = type_label.view(-1)
        predict_criterion = nn.CrossEntropyLoss()
        predict_loss = predict_criterion(type_predict, type_label)

        AT_net_loss = policy_loss - entropy_loss*self.entropy_coef # + probs_loss*20
        CT_net_loss = value_loss * 1.0 + predict_loss*self.predict_coef
