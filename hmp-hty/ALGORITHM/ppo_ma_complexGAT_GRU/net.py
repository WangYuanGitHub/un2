...
class I_GAT(nn.Module):
    '''
        修改后的complex版本,可以利用专门的数据来计算注意力权重
    '''
    def __init__(self, input_dim, hidden_dim, output_dim=0, n_heads=1, add_self=True, add_elu=True):
    ...
    def forward(self, h, a, mask=None):
        '''
            h是信息传递数据, a是权重计算数据
        '''
        assert self.input_dim == a.shape[-1]
        A = torch.matmul(a, self.W)
        B, N, C = A.size()  # B N h_dim
        a_input = torch.cat([A.repeat(1, 1, N).view(B, N * N, C), A.repeat(1, N, 1)], dim=2).view(B, N, N, 2*self.hidden_dim)  # [B,N,N,2C]
        e = self.act(torch.matmul(a_input, self.a).squeeze(-1)) # (B N N)
        ...
        weighted_sum = torch.matmul(attention, h) # (B N N) * (B N X) = (B N X) X为传递信息的维度
        assert not torch.isnan(weighted_sum).any(), ('nan problem!')

        if self.add_elu:
            H_I = F.elu(h + weighted_sum) if self.add_self else F.elu(weighted_sum)
        else:
            H_I = (h + weighted_sum) if self.add_self else (weighted_sum)
        return H_I
...
class Net(nn.Module):
    def __init__(self, rawob_dim, n_action, **kwargs):
        ...
        self.ct_encoder = nn.Sequential(nn.Linear(_size, h_dim), nn.ReLU(inplace=True),nn.Linear(h_dim, h_dim))
        self.ct_gru_out_mlp = nn.Sequential(nn.Linear(h_dim, h_dim), nn.ReLU(inplace=True),nn.Linear(h_dim, h_dim))
        # E_GAT参数
    ...
    def _act(self, obs=None, action_code=None, gru_cell_memory=None, test_mode=None, eval_mode=False, eval_actions=None, avail_act=None, agent_ids=None, eprsn=None):
        ...
        ct_att = self.ct_gru_out_mlp(gru_cell_output)

        # 计算信息
        ct_info = self.ct_encoder(ct_bac)
        ct_final = self.ct_GAT_layer(h=ct_info, a=ct_att)
        # ct_bac = self.ct_attention_layer(k=ct_bac,q=ct_bac,v=ct_bac)
        value = self.get_value(ct_final)
