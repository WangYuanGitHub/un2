...
class Net(nn.Module):
    def __init__(self, rawob_dim, n_action, **kwargs):
        ...
        self.num_ally = AlgorithmConfig.num_ally
        self.num_opp  = AlgorithmConfig.num_opp
        ...
        self.base = E_GAT(h_dim, h_dim, output_dim=h_dim)
    ...
    def _act(self, obs=None, action_code=None, gru_cell_memory=None, test_mode=None, eval_mode=False, eval_actions=None, avail_act=None, agent_ids=None, eprsn=None):
        ...
        # HAMA part
        ally_obs = baec[..., 0:1+self.num_ally, :]
        opp_obs = torch.cat([baec[..., 0:1, :], baec[..., 1+self.num_ally:, :]],dim=-2)

        if ally_obs.dim() == 4:
            ally_obs = ally_obs.view(B*N, ally_obs.shape[2], ally_obs.shape[3])
            opp_obs = opp_obs.view(B*N, opp_obs.shape[2], opp_obs.shape[3])
        ...
        else:             return value, actLogProbs, distEntropy, probs, others, type_predict, gru_cell_output
