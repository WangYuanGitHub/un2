...
class Net(nn.Module):
    def __init__(self, rawob_dim, n_action, **kwargs):
        ...
        self.Hybrid_Value_Head = nn.ModuleList([nn.Sequential(nn.Linear(h_dim, h_dim), nn.ReLU(inplace=True),nn.Linear(h_dim, 1)) for _ in range(self.num_type)])
    ...
    def _act(self, obs=None, action_code=None, gru_cell_memory=None, test_mode=None, eval_mode=False, eval_actions=None, avail_act=None, agent_ids=None, eprsn=None):
        ...
        logits = (hybrid_logits * repeat_at(Hybrid_Mask,-1,hybrid_logits.shape[-1])).sum(dim=0).squeeze()
        ...
        hybrid_value = torch.zeros(T, B, N, 1, device=obs.device)
        for i in range(self.num_type):
            hybrid_value[i,...] = self.Hybrid_Value_Head[i](ct_bac)
        value = (hybrid_value * repeat_at(Hybrid_Mask,-1,hybrid_value.shape[-1])).sum(dim=0)
...
#         mask_self = torch.zeros_like(ALL_TYPE_MASK[:,:, 0:1])
#         mask_ally = ALL_TYPE_MASK[:,:, 1:1+num_ally]
#         mask_opp =  ALL_TYPE_MASK[:,:, 1+num_ally:]
#         MASK_ALLY = torch.cat([mask_self,mask_ally],dim=-1)
#         MASK_OPP  = torch.cat([mask_self,mask_opp],dim=-1)
#         # 计算权重后张量
#         device = h.device
#         H_ALLY = torch.zeros(self.num_type, h_ally.shape[0], h_ally.shape[1], self.hidden_dim, device=device) # (T, B, N, C)
#         H_OPP = torch.zeros(self.num_type, h_opp.shape[0], h_opp.shape[1], self.hidden_dim, device=device)    # (T, B, N, C)
#                                 # 计算自己的张量时没有办法，只能for循环 self_type: (B)
#                                 # batch_size = H_ALLY.shape[1]      
#                                 # origin version
#                                 # H_self = torch.zeros_like(torch.matmul(h_self, self.W[0]))
#                                 # for i in range(batch_size):
#                                 #     temp_type = self_type[i]
#                                 #     temp_h_self = torch.matmul(h_self[i,:,:],self.W[temp_type])
#                                 #     H_self[i,:,:] = temp_h_self
#                                 # H_self = H_self.unsqueeze(0)                          # (B, 1, C) --> (1, B, 1, C)
#                                 # e.g. OPP_INPUT = torch.cat( [H_self.repeat(T,1,N,1), H_OPP], dim=-1).view(T, B, N, 2*C)
#         for i in range(self.num_type):
#             H_ALLY[i,:,:,:] = torch.matmul(h_ally, self.W[i])
#             H_OPP[i,:,:,:] = torch.matmul(h_opp, self.W[i])
#         # calculate ally embedding
#         T, B, N, C = H_ALLY.size()
#         ALLY_INPUT = torch.cat( [H_ALLY[:,:,0:1,:].repeat(1,1,N,1), H_ALLY], dim=-1).view(T, B, N, 2*C)  # (1, B, 1, C).repeat(T,1,N,1) --> (T, B, N, C)
#         E_ALLY = torch.zeros(self.num_type, B, N, 1, device=device)
#         for i in range(self.num_type):
#             E_ALLY[i,:,:,:] = torch.matmul(ALLY_INPUT[i,:,:,:], self.a[i]) # (type, batch, agent, 1)
#         E_ALLY = self.act(E_ALLY.squeeze(-1))  # (type, batch, agent, 1) --> (type, batch, agent) 
#         E_ALLY[MASK_ALLY.bool()] = -math.inf
#         E_ALLY_TYPE =  E_ALLY.view(B, N*T)     # (type, batch, agent) --> (batch, agent * type) 
#         ally_attention = F.softmax(E_ALLY_TYPE, dim=-1) #  (batch, agent * type) 

#         mask_ally = MASK_ALLY.view(B, N*T)
#         ally_attnc = ally_attention.clone()
#         ally_attnc[mask_ally] = 0
#         ally_attention = ally_attnc
#         # final matmul: (batch, 1, agent*type) * (batch, agent*type, h_dim) --> (batch, h_dim)
#         ally_weighted_sum = torch.matmul(ally_attention.unsqueeze(1),H_ALLY.view(B, N*T, C)).squeeze(1)

#         # calculate opp embedding
#         T, B, N, C = H_OPP.size()
#         OPP_INPUT = torch.cat( [H_OPP[:,:,0:1,:].repeat(1,1,N,1), H_OPP], dim=-1).view(T, B, N, 2*C)
#         E_OPP = torch.zeros(self.num_type, B, N, 1, device=device)
#         for i in range(self.num_type):
#             E_OPP[i,:,:,:] = torch.matmul(OPP_INPUT[i,:,:,:], self.a[i]) # (type, batch, agent, 1)
#         E_OPP = self.act(E_OPP.squeeze(-1))  # (type, batch, agent, 1) --> (type, batch, agent) 
#         E_OPP[MASK_OPP.bool()] = -math.inf
#         E_OPP_TYPE =  E_OPP.view(B, N*T)     # (type, batch, agent) --> (batch, agent * type) 
#         opp_attention = F.softmax(E_OPP_TYPE, dim=-1) #  (batch, agent * type) 

#         mask_opp = MASK_OPP.view(B, N*T)
#         opp_attnc = opp_attention.clone()
#         opp_attnc[mask_opp] = 0
#         opp_attention = opp_attnc
#         # final matmul: (batch, 1, agent*type) * (batch, agent*type, h_dim) --> (batch, h_dim)
#         opp_weighted_sum = torch.matmul(opp_attention.unsqueeze(1),H_OPP.view(B, N*T, C)).squeeze(1)

#         # origin version
#         # H_self = H_self.squeeze() # (1, B, 1, C) --> (B, C)

#         # assert self.add_self, ('Het-GAT need to add self info!')

#         # if self.add_elu:
#         #     H_E = F.elu(H_self + ally_weighted_sum + opp_weighted_sum) 
#         # else:
#         #     H_E = (H_self + ally_weighted_sum + opp_weighted_sum) 

#         # return H_E.view(B, self.hidden_dim)
#         assert self.add_self, ('Het-GAT need to add self info!')

#         if self.add_elu:
#             H_E = F.elu(ally_weighted_sum + opp_weighted_sum) 
#         else:
#             H_E = (ally_weighted_sum + opp_weighted_sum) 
