...
def _check_unsupported_spaces(env: gym.Env, observation_space: spaces.Space, action_space: spaces.Space) -> None:
    ...
    if isinstance(observation_space, spaces.Dict) and not isinstance(env, gym.GoalEnv):
        warnings.warn(
            ...
            "(cf https://*******.com/openai/gym/blob/master/gym/core.py), "
            "this is currently not supported by Stable Baselines "
            "(cf https://*******.com/hill-a/stable-baselines/issues/133), "
...
def _check_spaces(env: gym.Env) -> None:
    ...
    gym_spaces = " cf https://*******.com/openai/gym/blob/master/gym/spaces/"
...
def check_env(env: gym.Env, warn: bool = True, skip_render_check: bool = True) -> None:
    ...
    Please take a look at https://*******.com/openai/gym/blob/master/gym/core.py
    ...
    assert isinstance(
        env, gym.Env
    ), "Your environment must inherit from the gym.Env class cf https://*******.com/openai/gym/blob/master/gym/core.py"
