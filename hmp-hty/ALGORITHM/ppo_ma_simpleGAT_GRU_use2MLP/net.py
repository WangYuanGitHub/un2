...
class Net(nn.Module):
    def __init__(self, rawob_dim, n_action, **kwargs):
        ...
        self.GRU_obs_encoder = nn.Sequential(nn.Linear(_size, h_dim*2), nn.ReLU(inplace=True), nn.Linear(h_dim*2, h_dim))
        self.GRU_act_encoder = nn.Sequential(nn.Linear(self.n_action, h_dim), nn.ReLU(inplace=True), nn.Linear(h_dim, h_dim))
    ...
    def _act(self, obs=None, action_code=None, gru_cell_memory=None, test_mode=None, eval_mode=False, eval_actions=None, avail_act=None, agent_ids=None, eprsn=None):
        ...
        ct_act = action_code
        obs_emb = self.GRU_obs_encoder(ct_bac)
        act_emb = self.GRU_act_encoder(ct_act)
        gru_input = torch.cat((obs_emb, act_emb),-1)
