...
class ReinforceAlgorithmFoundation(RLAlgorithmBase):
    ...
    def action_making(self, StateRecall, test_mode):
        ...
        #     message_obs = torch.zeros([obs.shape[0], self.n_agent, self.n_agent, AlgorithmConfig.obs_h_dim])
        #     message_adv = torch.zeros([obs.shape[0], self.n_agent, self.n_agent, AlgorithmConfig.adv_h_dim])
        #     nn.init.xavier_uniform_(message_obs)
        #     nn.init.xavier_uniform_(message_adv)
        #     self.Message['obs'] = message_obs
        #     self.Message['adv'] = message_adv
