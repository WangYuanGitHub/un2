...
class ReinforceAlgorithmFoundation(RLAlgorithmBase):
    ...
    def action_making(self, StateRecall, test_mode):
        ...
        if self.OBS_LAST is None:
            shape = [obs.shape[0], obs.shape[1], obs.shape[2], AlgorithmConfig.net_hdim]
            self.OBS_LAST = np.random.rand(*shape)
        ...
        previous2now_obs_last = np.zeros([obs.shape[0], obs.shape[1], obs.shape[2], AlgorithmConfig.net_hdim])
        j = 0
        for i, is_active in enumerate(threads_active_flag):
            if is_active:
                ...
                previous2now_obs_last[j] = self.OBS_LAST[i]
                j += 1
        # make decision
        with torch.no_grad():
            action, value, action_log_prob, gru_cell_output, obs_last_output = self.policy.act(obs=obs,
                                                                              obs_last = previous2now_obs_last,
        ...
        for i, is_active in enumerate(threads_active_flag):
            if is_active:
                ...
                self.OBS_LAST[i] = obs_last_output[j]
                j += 1
