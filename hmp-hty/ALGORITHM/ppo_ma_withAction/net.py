...
class Net(nn.Module):
    def __init__(self, rawob_dim, n_action, **kwargs):
        ...
        _size = n_entity * h_dim
        self.ct_encoder = nn.Sequential(nn.Linear(_size + self.n_action, h_dim*2), nn.ReLU(inplace=True), nn.Linear(h_dim*2, h_dim))
    ...
    def _act(self, obs=None, action_code=None, gru_cell_memory=None, test_mode=None, eval_mode=False, eval_actions=None, avail_act=None, agent_ids=None, eprsn=None):
        ...
        # at_act = action_code 
        # policy_net_input = torch.cat((at_bac, at_act),-1)
        # logits = self.policy_head(policy_net_input)
        ...
        ct_act = action_code
        input = torch.cat((ct_bac, ct_act),-1)
