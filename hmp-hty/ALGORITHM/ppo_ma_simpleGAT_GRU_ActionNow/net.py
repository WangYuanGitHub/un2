...
class Net(nn.Module):
    ...
    def _act(self, obs=None, action_code=None, gru_cell_memory=None, test_mode=None, eval_mode=False, eval_actions=None, avail_act=None, agent_ids=None, eprsn=None):
        ...
        # 提取action_code
        # with torch.no_grad():
        #     action_code_now = torch.eye(AlgorithmConfig.n_action)[act].squeeze(1).to(GlobalConfig.device)
        action_code_now = torch.eye(AlgorithmConfig.n_action)[act].squeeze(1).detach().to(GlobalConfig.device)
        # # # # # # # # # # critic # # # # # # # # # # # #
        ct_bac = my_view(baec,[0,0,-1])
        ct_act = action_code_now
