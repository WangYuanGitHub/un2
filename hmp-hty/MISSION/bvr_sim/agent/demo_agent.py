...
"""
from agent.Yi_team.UTIL.colorful import *
...
class DemoAgent(Agent):
    ...
    def init_pos(self, cmd_list):
        ...
        for leader in self.my_manned_info: # 为己方有人机设置初始位置
            ...
            # 400 是有人机最大速度, init_speed=[150, 400], init_heading=[0,360]
            cmd_list.append(CmdEnv.make_entityinitinfo(
                leader['ID'], 
                leader_original_pos['X'], 
                leader_original_pos['Y'], 
                leader_original_pos['Z'],
                400,    # init_speed
                45))    # init_heading
            print亮紫('make_entityinitinfo()',leader['ID'], leader_original_pos['X'], leader_original_pos['Y'], leader_original_pos['Z'],400, 45)
        ...
        for sub in self.my_uvas_infos: # 为己方每个无人机设置初始位置
            ...
            if sub_index & 1 == 0: # 将当前编号放在有人机的一侧
                ...
                cmd_list.append(CmdEnv.make_entityinitinfo(
                    sub['ID'], 
                    sub_pos['X'], 
                    sub_pos['Y'] + interval_distance, # y轴分布开
                    sub_pos['Z'],
                    300 * 0.6, # 速度
                    45))
                print紫('make_entityinitinfo()',sub['ID'], sub_pos['X'], sub_pos['Y'] + interval_distance, sub_pos['Z'],300 * 0.6, 45)
            else:                   # 将当前编号放在有人机的另一侧
                # CmdEnv.make_entityinitinfo 指令，可以将 飞机实体置于指定位置点，参数依次为 实体ID，X坐标，Y坐标，Z坐标，初始速度，初始朝向
    ...
    def process_attack(self, sim_time, cmd_list):
        ...
        :param cmd_list保存所有的决策完毕任务指令列表
        """
        # 遍历所有的己方飞机，选择离己方最近距离的飞机进行攻击
        for plane_info in self.my_allplane_infos:  
            if plane_info['LeftWeapon'] == 0: continue# 没有导弹，不用处理
            
            # 还有导弹，LeftWeapon飞机的导弹数量
            close_target = self.select_close_target(
                plane_info, 
                self.enemy_allplane_infos
            ) # 选取 最近 的 一个敌机

            if len(close_target) < 1: continue # 如果没有附近（小于450000）的敌方飞机
                
            if close_target['close_distance'] <= 80e3: #当最近的目标离己方飞机的间距小于60000时，80km
                ...
                if plane_info['Type']==1:
                    print亮黄('make_attackparam()',plane_info['ID'], close_target['enemy_data']['ID'], 1)
                else:
                    print黄('make_attackparam()',plane_info['ID'], close_target['enemy_data']['ID'], 1)

                #保存已经攻击过的目标, 用于脱离巡航状态
                self.attack_handle_enemy[int(plane_info['ID'])]=close_target['enemy_data']['ID'] 
    ...
    def mission_start(self, sim_time, cmd_list):
        """
         ...
         :param cmd_list保存所有的决策完毕任务指令列表
         """
        for plane_info in self.my_uvas_infos:  # 遍历所有的己方“无人”飞机
            if len(self.enemy_manned_info) > 0:
                ...
                cmd_list.append(CmdEnv.make_linepatrolparam(
                    plane_info['ID'], 
                    leader_fire_route_list, #里面是敌方有人机的位置
                    data['move_max_speed'],
                    data['move_max_acc'],
                    data["move_max_g"]))
                if plane_info['Type']==1:
                    print亮蓝('make_linepatrolparam()',plane_info['ID'], leader_fire_route_list, data['move_max_speed'],data['move_max_acc'],data["move_max_g"])
                else:
                    print蓝('make_linepatrolparam()',plane_info['ID'], leader_fire_route_list, data['move_max_speed'],data['move_max_acc'],data["move_max_g"])

    def process_move(self, sim_time, cmd_list):
        """
        机动 
        在初始化之后，令有人机t>300后再朝敌方机动
        令无人机立即机动
        ...
        for plane in self.my_allplane_infos:# 拿到当前有人机信息
            # 如果当前飞机机处于“等待攻击”状态
            if plane['ID'] in self.attack_handle_enemy: continue
            # 如果当前飞机机不处于“等待攻击”状态
            ...
            if len(self.enemy_manned_info) > 0:
                ...
                if sim_time % 10 == 0:
                    ...
                    cmd_list.append(CmdEnv.make_motioncmdparam(
                        plane['ID'], 
                        1, # 1, 指令加速度
                        speed, 
                        data['move_max_acc'],   # 最大加速度
                        data['move_max_g']))
                    if plane['Type']==1:
                        print亮紫('make_motioncmdparam()',plane['ID'], 1, speed, data['move_max_acc'], data['move_max_g'])
                    else:
                        print紫('make_motioncmdparam()',plane['ID'], 1, speed, data['move_max_acc'], data['move_max_g'])
                # sim_time大
                elif plane['Type'] == 1 and sim_time > 300:#　一段时间后，跟随对方有人机
                    # CmdEnv.make_followparam 指令，可以令飞机跟随另一架飞机（敌我），参数依次为 飞机ID，跟随实体ID ，跟随速度，跟随加速度，跟随超载（与飞机转向角度有关）
                    cmd_list.append(CmdEnv.make_followparam(
                        plane['ID'], 
                        enemy_leader['ID'], 
                        data['move_max_speed'], 
                        data['move_max_acc'], 
                        data['move_max_g']))
                    if plane['Type']==1:
                        print亮紫('make_followparam()',plane['ID'], enemy_leader['ID'], data['move_max_speed'], data['move_max_acc'], data['move_max_g'])
                    else:
                        print紫('make_followparam()',plane['ID'], enemy_leader['ID'], data['move_max_speed'], data['move_max_acc'], data['move_max_g'])
                # 其他时间？
                ...
                cmd_list.append(    # 让飞机围绕当前位置巡逻
                    CmdEnv.make_areapatrolparam(
                        plane['ID'], 
                        plane['X'],             # 区域中心 x 轴坐标
                        plane['Y'],             # 区域中心 y 轴坐标
                        data['area_max_alt'],   # 区域 z 轴坐标
                        200,                    # 区域长度
                        100,                    # 区域宽度
                        data['move_max_speed'], 
                        data['move_max_acc'], 
                        data['move_max_g']))
                if plane['Type']==1:
                    print亮紫('make_areapatrolparam()',plane['ID'], plane['X'], plane['Y'], data['area_max_alt'], 200, 100, data['move_max_speed'], data['move_max_acc'], data['move_max_g'])
                else:
                    print紫('make_areapatrolparam()',plane['ID'], plane['X'], plane['Y'], data['area_max_alt'], 200, 100, data['move_max_speed'], data['move_max_acc'], data['move_max_g'])

    def select_close_target(self,own_plane,enemy_list)->{}:
        """
        选择离己方飞机最近的敌方飞机
